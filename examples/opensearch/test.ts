...
// const disposableDataLoader = <T extends (...args) => any>(batchFunction: T) => {
//   const loader = new DataLoader<string, Awaited<ReturnType<T>>>(batchFunction);
//   return {
//     [Symbol.dispose]: () => {
//       console.log(loader)
//       loader.clearAll();
...
const disposableDataLoader = <T extends DataLoader<any, any>>(loader: T) => {
  return {
    [Symbol.dispose]: () => {
      console.log(loader)
      loader.clearAll();
...
const loader = new DataLoader<string, Task>(async (ids) => {
  return ids.map((id) => tasks.find((task) => task.id === id)) as Task[];
});
const taskLoader = disposableDataLoader(loader)
// Example usage
const projectsLoader = new DataLoader(async (ids) => {


  using disposableTaskLoader = taskLoader;

  const userLoader = new DataLoader<string, User>(async (ids) => {
    return ids.map((id) => users.find((user) => user.id === id)) as User[];
  });

  const projectLoader = new DataLoader<string, Project>(
    async (ids) => {
      return ids.map((id) => projects.find((project) => project.id === id)) as Project[];
    }
  );
    ...
    ids.map((id) =>
      loadRelations(ProjectSchema, id as string, {
        ...
        relations: {
          ...
          tasks: {
            field: "taskIds",
            loader: disposableTaskLoader.loader,
          },
          members: {
            field: "memberIds",
            loader: userLoader,
